...
> 공통 원칙·CLI 규약·`result.json` 표준은 [00_overview.md](./00_overview.md) 단일 출처를 따른다.
> Phase별 상세 구현 순서는 [StateTree Bridge 상세 구현 가이드](./ue5_statetree_automation_bridge_plan.md) 참조.
...
- Agent는 판단 / 스펙 작성 / 명령 생성 / 리포트 해석을 담당한다.
- UE Editor Plugin 또는 Commandlet은 실제 UE API 접근을 담당한다.
- `ue-auto` CLI는 Agent와 UE Plugin 사이의 얇은 브릿지 역할만 한다.
...
| `Saved/AutomationReports/statetree.validation.json` | Agent가 읽는 검증 결과 |
...
- Agent가 다음 명령을 만들 수 있을 정도로 구조화된 JSON을 제공한다.
...
Agent는 직접 `.uasset`을 수정하지 않고, 다음 형태의 루프만 수행한다.
...
| Agent가 과도한 명령 생성 | dry-run 기본값, apply는 사용자 승인 후 |
